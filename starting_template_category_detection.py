...
def main(args):
    ...
    # --- EXP NAME CREATION
    name = exp_name(args)
    result_path = './results/category_detection_results/{}'.format(name)
    ...
    # --- METRICS AND LOGGING
    mandatory_metrics = [make_ego_objects_metrics(save_folder=result_path, filename_prefix='track2_output')]
    ...
    evaluator = EvaluationPlugin(
        ...
        loggers=[InteractiveLogger(),
                 TensorboardLogger(tb_log_dir='./log/track_cat_det/exp_' + datetime.datetime.now().isoformat())],
        benchmark=benchmark
    )
