...
def main(args):
    ...
    mandatory_plugins = [
        DetectionMetrics(
            benchmark, save_folder='./results/instance_detection_results/{}'.format(name))
    ]
    ...
    evaluator = EvaluationPlugin(
        ...
        loggers=[InteractiveLogger(),
                 ...
                 WandBLogger(project_name='clvision-track3', run_name=name, save_code=False, sync_tfboard=True, dir='./results/'),
                 ],
        benchmark=benchmark
    )
    # ---------
    ...
    study_trial = 10 #몇 번 탐색할지
    def objective(trial: Trial, benchmark):
        # 탐색할 파라미터 구간
        param = {
            'learning_rate': trial.suggest_discrete_uniform('learning_rate', 0.01, 0.11, 0.05),
            'momentum': trial.suggest_discrete_uniform('momentum', 0.9, 0.99, 0.05)
        }
        ...
        optimizer = torch.optim.SGD(params,
                                    lr=param["learning_rate"], momentum=param["momentum"])
        train_mb_size = 4
    ...
    study.optimize(lambda trial: objective(trial, benchmark), n_trials=study_trial)
    ...
    params = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.SGD(params,
                                lr=best_param["learning_rate"], momentum=best_param["momentum"])
    train_mb_size = 4
